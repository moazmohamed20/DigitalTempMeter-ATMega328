...
{
  // In 8 MHz Crystal, Each Clock Pulse = 1/8 = 0.125 usec.
  // If We Use Prescaler Of 64, Each Clock Pulse = 0.125*64 = 8 usec.
  // So To Get 1 msec, We Need Counts = 1000/8 = 125
  OCR0A = counts;
...
void enable_adc(uint8_t channel)
{
  ...
  ADCSRA |= (1 << ADPS2) | (1 << ADPS1);              // Prescaler = 64 --> 8 MHz / 64 = 150 KHz
  ADCSRA |= (1 << ADEN) | (1 << ADSC) | (1 << ADATE); // Enable ADC In "Free Running Mode"
}
